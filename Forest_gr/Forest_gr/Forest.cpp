...
{
    Forest previous_forest = *this;
    int forest_height = int(previous_forest.forest_.GetHeight());
    int forest_width = int(previous_forest.forest_.GetWidth());
        ...
        for (int x = 0; x < forest_width; x++)
        {
            ...
            for (int i = y - 1; i <= y + 1; i++)
            {
                for (int j = x - 1; j <= x + 1; j++)
                {
                    ...
                    {
                        if (previous_forest.forest_.GetState(i, j) == TREE_IN_FIRE)
                        {
            ...
            int time = previous_forest.forest_.GetElapsedTime(y, x);
            int new_time = 0;
            tree_state state = previous_forest.forest_.GetState(y, x);
