...
void MessageBox::Create(
    ...
    const int char_size)
{
    // задать позицию для background_
    ...
    // задать фон для background_  (setFillColor)
    ...
    // (float)3 - C-style
    // static_cast<float>3 - делает то же самое, но исправляет недостатки первого
}
...
void MessageBox::Update(const float dt)
{
    ...
    const size_t visible_messages_count = static_cast<size_t>(
        (vertical_size - 2.0f) / line_height
        );
...
void MessageBox::Render(sf::RenderWindow& window)
{
    ...
    all_messages_.setPosition(sf::Vector2f(background_.getPosition().x + 5, background_.getPosition().y + 5));
