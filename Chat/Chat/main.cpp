...
//void InitSocketForWriting(SOCKET& socket_for_writing, const std::string& remote_ip_adress, const int& remote_port)
//{
//    assert(socket_for_writing);
...
void WriteAndSendMessages(
    ...
    const std::string& remote_ip_adress,
    const int remote_port)
{
    assert(1024 <= remote_port && remote_port <= 65535);
    SOCKET socket_for_writing = GetNewSocket();

    std::cerr << "***Connecting to " << remote_ip_adress
              << ":" << remote_port << "...***" << std::endl;
    InitSocketForWriting(socket_for_writing, remote_ip_adress, remote_port);
...
int main()
{
    ...
    // Run asynchronous receiving messages
    std::future<void> receiver_future = std::async(
        std::launch::async,
        ReceiveAndPrintMessages, std::ref(std::cout), my_port
    );
